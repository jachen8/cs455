...
class userThread extends Thread {
  ...
  public void run() {
    ...
    try {
      ...
      userName = input_stream.readLine(); //reads the user input for name
      if(userName.startsWith("#join")) {
        output_stream.println("#welcome " + userName.substring(5)); //server sends message to client
        output_stream.flush();
        System.out.println("Welcome " + userName.substring(5)); //client sends message to user
      }
      ...
      while (true) {
        ...
        //checks to see if user wants to leave chat
